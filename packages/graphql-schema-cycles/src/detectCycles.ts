export function detectCycles(Graph: any[], select: any) {

  function pruneEdge(SCC: any) {
    for(var component in SCC) {
      for(var vertex in SCC[component]) {
        for (let ref = 0; ref < SCC[component][vertex].referenceList.length; ref++) {
    ...
    function tarjan(input: any) {
      ...
      function strongConnect(vertex: any) {
        vertex.index = index;
        ...
        function johnson(component: any) {
          ...
          function findCycles (v: any) {
            ...
            for (var edge in component[v].referenceList) {
              let edgeRef = component[v].referenceList[edge];
              if (edgeRef.reference.jIndex === startvertex) {
                ...
                stackEdges.push(edgeRef.label);
                var foundCycle: any = findCycles(edgeRef.reference.jIndex);
              }
            }
